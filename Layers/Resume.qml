...
Scope {
  ...
  Connections {
    ...
    function onShouldShowChanged() {
      ...
      if (root.shouldShow) Info.NetInfo.scanNetworks()
    }
  }
  Wid.P3rTransition {
    id: resumeTransition
    ...
    PanelWindow {
      ...
      Item {
        ...
        Column {
          ...
          Repeater {
            model: [
              { badge: "I",   title: "Stats",    subtitle: "Repositories / Activity", rank: 3 },
              { badge: "II",  title: "Network",  subtitle: "Languages & Frameworks",  rank: 4 },
              { badge: "III", title: "Bluetooth", subtitle: "Featured Work",           rank: 5 },
            ]
            Item {
              id: cardWrap
              ...
              Canvas {
                ...
                Connections {
                  target: cardWrap
                  function onIsActiveChanged() { cardCanvas.requestPaint() }
                }
                Connections {
                  target: root
                  function onContentVisibleChanged() { if (root.contentVisible) cardCanvas.requestPaint() }
                }
              }
              ...
              Text {
                anchors { left: parent.left; leftMargin: 62; top: parent.top; topMargin: 14 }
              ...
              Row {
                ...
                Text {
                  text: "RANK"
                  font.family: "Glirock"; font.pixelSize: 22
                  color: cardWrap.isActive ? "#000" : "#9ffbff"
                  anchors.bottom: parent.bottom; bottomPadding: 8
                }
                Text {
                  text: cardWrap.modelData.rank
                  font.family: "Glirock"; font.pixelSize: 60
                  color: cardWrap.isActive ? "#000" : "#9ffbff"
                }
              }
              ...
              Canvas {
                id: subtitleCanvas
                anchors { left: parent.left; leftMargin: 64; right: parent.right; rightMargin: 14; bottom: parent.bottom; bottomMargin: 12 }
                height: 32
                onPaint: {
                  var ctx = getContext("2d")
                  ctx.clearRect(0, 0, width, height)
                  ...
                  ctx.lineTo(width - 10, height); ctx.lineTo(0, height)
                  ctx.closePath()
                ...
                Connections {
                  target: cardWrap
                  function onIsActiveChanged() { subtitleCanvas.requestPaint() }
                }
                Connections {
                  target: root
                  function onContentVisibleChanged() { if (root.contentVisible) subtitleCanvas.requestPaint() }
                }
                Text {
                  anchors { fill: parent; leftMargin: 14 }
                  text: cardWrap.modelData.subtitle
                  font.family: "Glirock"; font.pixelSize: 20
        ...
        Rectangle {
          id: detailPanel
          anchors {
            right: parent.right; rightMargin: parent.width * 0.045
            top: parent.top;    topMargin:   parent.height * 0.095
          }
          ...
          Canvas {
            ...
            Connections {
              target: root
              function onContentVisibleChanged() { if (root.contentVisible) panelBg.requestPaint() }
            }
            onPaint: {
              var ctx = getContext("2d")
              ctx.clearRect(0, 0, width, height)
              ctx.beginPath()
              ctx.moveTo(0, 0); ctx.lineTo(width, 0)
              ctx.lineTo(width - 18, height); ctx.lineTo(0, height)
              ctx.closePath()
          ...
          Canvas {
            ...
            Connections {
              target: root
              function onContentVisibleChanged() { if (root.contentVisible) detailHeader.requestPaint() }
            }
            onPaint: {
              var ctx = getContext("2d")
              ctx.clearRect(0, 0, width, height)
              ctx.beginPath()
              ctx.moveTo(0, 0); ctx.lineTo(width, 0)
              ctx.lineTo(width - 16, height); ctx.lineTo(0, height)
              ctx.closePath()
            ...
            Row {
              ...
              spacing: 14
              Text { text: detailHeader.indexText; font.family: "Glirock"; font.pixelSize: 40; color: "#08153f"; anchors.verticalCenter: parent.verticalCenter }
              Text { text: detailHeader.titleText; font.family: "Glirock"; font.pixelSize: 36; color: "#08153f"; anchors.verticalCenter: parent.verticalCenter }
            }
          }

          ScrollView {
            anchors { left: parent.left; right: parent.right; top: detailHeader.bottom; bottom: parent.bottom; topMargin: 18 }
            clip: true
            ...
            Column {
              ...
              Repeater {
                model: {
                  ...
                  return [
                    [
                      { title: "OS",    status: Info.SysInfo.osName },
                      { title: "CPU",   status: Math.round(Info.SysInfo.cpuUsage * 100) + "%" },
                      { title: "RAM",   status: Info.SysInfo.memText },
                      { title: "DISK",  status: Info.SysInfo.diskText },
                      { title: "Users", status: Info.SysInfo.loggedInUsers },
                    ],
                    ...
                    Info.BluetoothInfo.friendlyDeviceList.length === 0
                      ? [{ title: Info.BluetoothInfo.available ? "Bluetooth Off" : "No Adapter", status: Info.BluetoothInfo.enabled ? "No Devices" : "Disabled" }]
                      : Info.BluetoothInfo.friendlyDeviceList.map(d => ({
                          title:  d.name,
                          status: d.connected ? "Connected" : (d.paired ? "Paired" : "Found")
                        })),
                    [
                      { title: "Open Source",       status: "Active"  },
                      { title: "Quickshell Themes", status: "Ongoing" },
                      { title: "NixOS Configs",     status: "Active"  },
                      { title: "Shader Research",   status: "Ongoing" },
                    ],
                  ][ac]
                }
                ...
                Item {
                  ...
                  Row {
                    ...
                    Text {
                      ...
                      font.family: "Glirock"; font.pixelSize: 24; color: "#f2fcff"
                    ...
                    Item {
                      ...
                      Canvas {
                        ...
                        Connections {
                          target: root
                          function onContentVisibleChanged() { if (root.contentVisible) statusCanvas.requestPaint() }
                        }
                        onPaint: {
                          var ctx = getContext("2d")
                          ctx.clearRect(0, 0, width, height)
                          ctx.beginPath()
                          ctx.moveTo(0, 0); ctx.lineTo(width, 0)
                          ctx.lineTo(width - 8, height); ctx.lineTo(0, height)
                          ctx.closePath()
                      ...
                      Text {
                        ...
                        font.family: "Glirock"; font.pixelSize: 16
                        color: "#06133b"
                      }
                    }
                  }
